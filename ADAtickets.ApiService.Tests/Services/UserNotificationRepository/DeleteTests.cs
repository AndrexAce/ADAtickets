...
namespace ADAtickets.ApiService.Tests.Services.UserNotificationRepository
{
    ...
    public sealed class DeleteTests
    {
        ...
        public async Task DeleteUserByIdAsync_ExistingEntity_DeletesEntity()
        {
            ...
            UserNotification userNotification = new() { Id = Guid.NewGuid(), ReceiverUserId = user.Id, NotificationId = notification.Id };
            ...
            _ = mockSet.Setup(s => s.Remove(It.IsAny<UserNotification>()))
                .Callback<UserNotification>(userNotification => userNotifications.RemoveAll(un => un.Id == userNotification.Id));
            ...
            UserNotification? deletedUserNotification = await mockContext.Object.UserNotifications.SingleOrDefaultAsync(cancellationToken);
