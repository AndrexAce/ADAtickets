...
public sealed class EditsController(IEditRepository editRepository, IMapper mapper, IHubContext<EditsHub> editsHub) : ControllerBase
{
    ...
    public async Task<ActionResult<EditResponseDto>> GetEdit(Guid id)
    {
        ...
        if (await editRepository.GetEditByIdAsync(id) is not Edit edit) return NotFound();
    ...
    public async Task<ActionResult<EditResponseDto>> PutEdit(Guid id, EditRequestDto editDto)
    {
        ...
        if (await editRepository.GetEditByIdAsync(id) is not Edit edit) return await PostEdit(editDto);
        ...
        catch (DbUpdateConcurrencyException)
        {
            ...
            if (await editRepository.GetEditByIdAsync(id) is null) return NotFound();
