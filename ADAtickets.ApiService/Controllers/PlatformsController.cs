...
/// <param name="mapper">
///     Object definining the mappings of fields between the <see cref="Platform" /> entity and its
///     <see cref="PlatformRequestDto" /> or <see cref="PlatformResponseDto" /> correspondant.
/// </param>
[Route($"v{Service.APIVersion}/{Controller.Platforms}")]
[ApiController]
...
public sealed class PlatformsController(IPlatformRepository platformRepository, IMapper mapper) : ControllerBase
{
    ...
    public async Task<ActionResult<PlatformResponseDto>> GetPlatform(Guid id)
    {
        ...
        if (await platformRepository.GetPlatformByIdAsync(id) is not Platform platform) return NotFound();
    ...
    public async Task<ActionResult<PlatformResponseDto>> PutPlatform(Guid id, PlatformRequestDto platformDto)
    {
        ...
        catch (DbUpdateConcurrencyException)
        {
            ...
            if (await platformRepository.GetPlatformByIdAsync(id) is null) return NotFound();
    ...
    public async Task<ActionResult<PlatformResponseDto>> PostPlatform(PlatformRequestDto platformDto)
    {
        ...
        return CreatedAtAction(nameof(GetPlatform), new { id = platform.Id },
            mapper.Map<PlatformResponseDto>(platform));
    }
