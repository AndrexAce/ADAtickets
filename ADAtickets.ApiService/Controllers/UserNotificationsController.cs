...
) : ControllerBase
{
    ...
    ///     <c>GET /api/UserNotifications?userId=123e4567-e89b-12d3-a456-426614174000&amp;pageNumber=1&amp;pageSize=10</c>
    ...
    public async Task<ActionResult<Page<UserNotificationResponseDto>>> GetUserNotifications(
        ...
        [FromQuery] Dictionary<string, string>? filters = null)
    {
        ...
        var userNotificationDtos = userNotifications.Select(mapper.Map<UserNotificationResponseDto>);

        // Return paginated or unpaginated results based on parameters
        Page<UserNotificationResponseDto> result;

        if (pageNumber.HasValue && pageSize.HasValue)
        {
            result = new Page<UserNotificationResponseDto>(userNotificationDtos, pageNumber.Value, pageSize.Value);
        }
        else
        {
            result = new Page<UserNotificationResponseDto>(userNotificationDtos);
        }
