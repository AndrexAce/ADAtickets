...
namespace ADAtickets.ApiService.Controllers
{
    ...
    /// <param name="mapper">Object definining the mappings of fields between the <see cref="UserNotification"/> entity and its <see cref="UserNotificationRequestDto"/> or <see cref="UserNotificationResponseDto"/> correspondant.</param>
    ...
    public sealed class UserNotificationsController(IUserNotificationRepository userNotificationRepository, IMapper mapper) : ControllerBase
    {
        /// <summary>
        ...
        /// </remarks>
        /// <param name="filters">A group of key-value pairs defining the property name and value <see cref="UserNotification"/> entities should be filtered by.</param>
        ...
        public async Task<ActionResult<IEnumerable<UserNotificationResponseDto>>> GetUserNotifications([FromQuery] Dictionary<string, string>? filters)
        {
            IEnumerable<UserNotification> userNotifications = await (filters != null ? userNotificationRepository.GetUserNotificationsByAsync(filters) : userNotificationRepository.GetUserNotificationsAsync());
        ...
        public async Task<ActionResult<UserNotificationResponseDto>> PutUserNotification(Guid id, UserNotificationRequestDto userNotificationDto)
        {
            ...
            try
            {
                ...
                await userNotificationRepository.UpdateUserNotificationAsync(mapper.Map(userNotificationDto, userNotification));
            }
            catch (DbUpdateConcurrencyException)
            {
                // If the entity is not found in the data source, it was deleted by another user while updating.
                if (await userNotificationRepository.GetUserNotificationByIdAsync(id) is null)
                {
                    return NotFound();
                }
        ...
        [HttpPost]
        [RequiredScope(Scope.Read, Scope.Write)]
        public async Task<ActionResult<UserNotificationResponseDto>> PostUserNotification(UserNotificationRequestDto userNotificationDto)
        {
            UserNotification userNotification = mapper.Map<UserNotification>(userNotificationDto);
            ...
            return CreatedAtAction(nameof(GetUserNotification), new { id = userNotification.Id }, mapper.Map<UserNotificationResponseDto>(userNotification));
        }
