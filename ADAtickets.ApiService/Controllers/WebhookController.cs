...
) : ControllerBase
{
    ...
    public async Task<IActionResult> PutTicket([FromBody] AzureDevOpsWebHookResponseDto payload)
    {
        ...
        var requesterId = await GetUserIdFromWorkItemAsync(payload, true);
    ...
    public async Task<IActionResult> DeleteTicket([FromBody] AzureDevOpsWebHookResponseDto payload)
    {
        ...
        // Check if this work item was edited by ADAtickets API to avoid infinite loops
        if (IsDeletedOrRepliedByADAtickets(payload))
        {
            return Ok();
        }
    ...
    public async Task<IActionResult> PostReply([FromBody] AzureDevOpsWebHookResponseDto payload)
    {
        ...
        // Check if this work item was replied by ADAtickets API to avoid infinite loops
        if (IsDeletedOrRepliedByADAtickets(payload))
        {
            return Ok();
        }
    ...
    private bool IsDeletedOrRepliedByADAtickets(AzureDevOpsWebHookResponseDto payload)
    {
    ...
    private async Task<Reply> CreateReplyFromWorkItemAsync(AzureDevOpsWebHookResponseDto payload, Ticket existingTicket)
    {
        ...
        var reply = new Reply
        {
            ...
            AuthorUserId = await GetUserIdFromWorkItemAsync(payload, false),
    ...
    private async Task<Guid> GetUserIdFromWorkItemAsync(AzureDevOpsWebHookResponseDto payload, bool isUpdate)
    {
        var userIdentity = isUpdate ? payload.Resource.Revision.Fields[AzureDevOpsWebHookResponseDto.WorkItemFields.ChangedBy].ToString()!
            : payload.Resource.Fields[AzureDevOpsWebHookResponseDto.WorkItemFields.ChangedBy].ToString()!;
