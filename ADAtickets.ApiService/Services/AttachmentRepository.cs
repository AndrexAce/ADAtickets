...
internal sealed class AttachmentRepository(ADAticketsDbContext context) : IAttachmentRepository
{
    ...
    public async Task<IEnumerable<Attachment>> GetAttachmentsByAsync(IEnumerable<KeyValuePair<string, string>> filters)
    {
        ...
        foreach (var filter in filters)
            ...
            {
                case nameof(Attachment.Id) when Guid.TryParse(filter.Value, out var outId):
                    query = query.Where(attachment => attachment.Id == outId);
                    break;

                case nameof(Attachment.TicketId) when Guid.TryParse(filter.Value, out var outTicketId):
    ...
    {
        var fullPath = await SaveAttachmentToFileSystemAsync(attachment.Path, data);
    ...
    {
        var fullPath = await ReplaceAttachmentInFileSystemAsync(attachment.Path, data, oldAttachmentPath);
    ...
    ///     <para>
    ///         If <paramref name="attachmentData" /> is empty, the method will save an empty file without launching
    ///         exceptions.
    ///     </para>
    /// </summary>
    ...
    /// <returns>
    ///     A <see cref="Task" /> returning the full path if the attachment was successfully saved, or
    ...
    private static async Task<string?> SaveAttachmentToFileSystemAsync(string attachmentName, byte[] attachmentData)
    {
        try
        {
            var baseDirectory = Path.Combine(Directory.GetCurrentDirectory(), "media");
            var saveDirectoryPath = Path.Combine(baseDirectory, DateTime.UtcNow.Year.ToString(),
                DateTime.UtcNow.Month.ToString(), DateTime.UtcNow.Day.ToString());

            _ = Directory.CreateDirectory(saveDirectoryPath);

            var fullPath = Path.Combine(saveDirectoryPath, attachmentName);
    ...
    private static bool DeleteAttachmentFromFileSystem(string attachmentPath)
    {
        try
        {
            if (!Path.IsPathRooted(attachmentPath)) return false;
    ...
    /// <returns>
    ///     A <see cref="Task" /> returning the full path if the attachment was successfully replaced, or
    ///     <see langword="null" /> otherwise.
    /// </returns>
    private static async Task<string?> ReplaceAttachmentInFileSystemAsync(string attachmentName, byte[] attachmentData,
        string oldAttachmentPath)
    {
        return DeleteAttachmentFromFileSystem(oldAttachmentPath)
            ? await SaveAttachmentToFileSystemAsync(attachmentName, attachmentData)
            : null;
    }
}