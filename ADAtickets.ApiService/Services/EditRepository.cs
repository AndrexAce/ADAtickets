...
internal sealed class EditRepository(ADAticketsDbContext context) : IEditRepository
{
    ...
    public async Task<IEnumerable<Edit>> GetEditsByAsync(IEnumerable<KeyValuePair<string, string>> filters)
    {
        ...
        foreach (var filter in filters)
            ...
            {
                case nameof(Edit.Id) when Guid.TryParse(filter.Value, out var outId):
                    query = query.Where(edit => edit.Id == outId);
                    break;

                case nameof(Edit.TicketId) when Guid.TryParse(filter.Value, out var outTicketGuid):
                    query = query.Where(edit => edit.TicketId == outTicketGuid);
                    break;

                case nameof(Edit.UserId) when Guid.TryParse(filter.Value, out var outUserGuid):
                ...
                case nameof(Edit.EditDateTime) when DateTimeOffset.TryParse(filter.Value, CultureInfo.InvariantCulture,
                    out var outEditDateTime):
