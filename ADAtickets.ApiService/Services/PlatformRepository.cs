...
/// </summary>
internal sealed class PlatformRepository(ADAticketsDbContext context) : IPlatformRepository
{
    ...
    public async Task<IEnumerable<Platform>> GetPlatformsByAsync(IEnumerable<KeyValuePair<string, string>> filters)
    {
        ...
        foreach (var filter in filters)
            ...
            {
                case nameof(Platform.Id) when Guid.TryParse(filter.Value, out var outId):
                    query = query.Where(platform => platform.Id == outId);
                    break;
