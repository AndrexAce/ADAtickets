...
internal class UserPlatformRepository(ADAticketsDbContext context) : IUserPlatformRepository
{
    ...
    public async Task<IEnumerable<UserPlatform>> GetUserPlatformsByAsync(
        IEnumerable<KeyValuePair<string, string>> filters)
    {
        ...
        foreach (var filter in filters)
            ...
            {
                case nameof(UserPlatform.Id) when Guid.TryParse(filter.Value, out var outId):
                    query = query.Where(u => u.Id == outId);
                    break;

                case nameof(UserPlatform.UserId) when Guid.TryParse(filter.Value, out var outUserId):
                    query = query.Where(u => u.UserId == outUserId);
                    break;

                case nameof(UserPlatform.PlatformId) when Guid.TryParse(filter.Value, out var outPlatformId):
