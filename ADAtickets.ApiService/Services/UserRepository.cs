...
/// </summary>
internal sealed class UserRepository(ADAticketsDbContext context) : IUserRepository
{
    ...
    public async Task<IEnumerable<User>> GetUsersByAsync(IEnumerable<KeyValuePair<string, string>> filters)
    {
        ...
        foreach (var filter in filters)
            ...
            {
                case nameof(User.Id) when Guid.TryParse(filter.Value, out var outId):
                ...
                case nameof(User.AreEmailNotificationsEnabled)
                    when bool.TryParse(filter.Value, out var outAreEmailNotificationsEnabled):
