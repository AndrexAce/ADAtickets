...
public sealed class PutTests
{
    ...
    public async Task UpdateEdit_ValidEntity_ReturnsNew(Edit inEdit)
    {
        ...
        [
            new()
            {
                Id = inEdit.Id, Description = "Old description.", TicketId = Guid.AllBitsSet, UserId = Guid.AllBitsSet
            }
        ];
        ...
        _ = mockEditSet.Setup(s => s.Update(It.IsAny<Edit>()))
            .Callback<Edit>(e =>
            {
                if (e.Description.Length <= 200 && mockTicketSet.Object.Single().Id == e.TicketId &&
                    mockUserSet.Object.Single().Id == e.UserId) edits[0].Description = inEdit.Description;
            });
