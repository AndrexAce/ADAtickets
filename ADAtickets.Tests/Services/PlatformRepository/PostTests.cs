...
public sealed class PostTests
{
    ...
    [
        Utilities.CreatePlatform(new string('a', 255), "https://example.com"),
        Utilities.CreatePlatform("Name", "://example.com")
    ];

    public static TheoryData<Platform> ValidPlatformData =>
    [
        Utilities.CreatePlatform("Name", "https://example.com")
    ];
    ...
    public async Task AddPlatform_ValidEntity_ReturnsPlatform(Platform inPlatform)
    {
        ...
        _ = mockPlatformSet.Setup(s => s.Add(It.IsAny<Platform>()))
            .Callback<Platform>(p =>
            {
                if (p.Name.Length <= 254 && Regex.IsMatch(p.RepositoryUrl,
                        @"^(https?:\/\/)?(www\.)?([a-zA-Z0-9\-]+\.)+[a-zA-Z]{2,}\/?$")) platforms.Add(p);
            });
