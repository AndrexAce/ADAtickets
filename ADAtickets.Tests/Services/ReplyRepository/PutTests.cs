...
public sealed class PutTests
{
    ...
    [
        Utilities.CreateReply(new string('a', 5001), Guid.AllBitsSet, Guid.AllBitsSet),
        Utilities.CreateReply("Valid message.", Guid.Empty, Guid.AllBitsSet),
        Utilities.CreateReply("Valid message.", Guid.AllBitsSet, Guid.Empty)
    ];

    public static TheoryData<Reply> ValidReplyData =>
    [
        Utilities.CreateReply("Valid message.", Guid.AllBitsSet, Guid.AllBitsSet)
    ];
    ...
    public async Task UpdateReply_ValidEntity_ReturnsNew(Reply inReply)
    {
        ...
        [
            new()
            {
                Id = inReply.Id, Message = "Old message.", AuthorUserId = Guid.AllBitsSet, TicketId = Guid.AllBitsSet
            }
        ];
        ...
        _ = mockReplySet.Setup(s => s.Update(It.IsAny<Reply>()))
            .Callback<Reply>(r =>
            {
                if (r.Message.Length <= 5000 && mockTicketSet.Object.Single().Id == r.TicketId &&
                    mockUserSet.Object.Single().Id == r.AuthorUserId) replies[0].Message = inReply.Message;
            });
