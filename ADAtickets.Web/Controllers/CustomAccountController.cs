...
public sealed class CustomAccountController(IConfiguration configuration) : Controller
{
    ...
    /// </summary>
    /// <remarks>
    ///     The <see cref="AccountController.SignOut(string)" /> method is not used since it does not allow to specify a
    ///     custom cookie signout scheme.
    /// </remarks>
    ...
    {
        var tid = HttpContext.User.GetTenantId();

        var scheme = tid == configuration["Entra:TenantId"]
